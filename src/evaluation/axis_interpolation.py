...
def evaluate_targets(c,*,output_override=None):
 ...
 selected=[x for x in all_rows if x["selected"]]; base=[x for x in all_rows if x["alpha"]==0.0]; keys=("auroc","auprc","balanced_accuracy","macro_f1","mcc"); means={k:{"alpha0":float(np.mean([x[k] for x in base])),"selected":float(np.mean([x[k] for x in selected]))} for k in keys}; improved_targets=sum((s["auroc"]>=b["auroc"] and s["auprc"]>=b["auprc"] and (s["auroc"]>b["auroc"] or s["auprc"]>b["auprc"])) for s,b in zip(selected,base)); ranking=means["auroc"]["selected"]>means["auroc"]["alpha0"] and means["auprc"]["selected"]>means["auprc"]["alpha0"]; operating=sum(means[k]["selected"]>means[k]["alpha0"] for k in ("balanced_accuracy","macro_f1","mcc")); status="strong_success" if ranking and operating>=2 and improved_targets>=2 else ("partial_success" if ranking else ("failure" if means["auroc"]["selected"]<means["auroc"]["alpha0"] and means["auprc"]["selected"]<means["auprc"]["alpha0"] and operating==0 else "mixed_or_neutral")); result={"experiment":c["experiment"],"selected_alpha":selection["selected_alpha"],"source_oof_threshold":selection["selected_threshold"],"target_specific_tuning":False,"target_labels_used_only_after_score_freeze":True,"per_target":per,"three_target_means":means,"targets_with_noninferior_and_improved_ranking":int(improved_targets),"m1_status":status}; _json(root/"analysis_result.json",result); _csv(root/"target_metrics.csv",all_rows); _json(root/"run_manifest.json",{"git":git_identity(),"analysis_result_sha256":sha256_file(root/"analysis_result.json"),"target_metrics_sha256":sha256_file(root/"target_metrics.csv")}); return result
